...
from population_selectors import tournment_selection,getrepulsorlistdistancerms,repulsortournment_distances01
...
def exec_GP_repulsors_04(runparameters, _):

    # uses simple distance with repulsor distance queficient
    ...
    for n in range (1,runparameters['MAX_GENERATIONS']+1):  #Repeat iterations
        ...
        print(str(runparameters['METHOD']) + " : " + str(runparameters['RUN_ID']) + '----------------------' + str(n) + " / Dictionary size : " + str(len(soldic)) )
        #debug
        iterbestelement[:] = []
        iterbestelementfitness=float(inf)#10*10**250 #very big and unfit number
        ...
        if len(wallofshame)>0:
            for m in range(0, int(runparameters['POPULATION_SIZE'])): # this is no good !
                repulsorchampion[:] = []
                repulsorchampion=repulsortournment_distances01(population,trainingset,runparameters['REPULSOR_TOURNMENT_SIZE'],wallofshame)
