...
####                     GP REPULSORS 07                              ####
####                                                                  ####
#### Normal tournement + repulsor tournement using euclidian distance ####
####                                                                  ####
##########################################################################
"""
#importar ficheiros
...
def exec_GP_repulsors_07(runparameters, _):

    #uses euclidean distance for repulsor tournement
    ...
    for n in range (1,runparameters['MAX_GENERATIONS']+1):  #Repeat iterations
        ...
        print(str(runparameters['METHOD']) + " : " + str(runparameters['RUN_ID']) + ' (' + str(runparameters['OVERFITTHRESHOLD']) + ') ----------------------' + str(n) + " / Dictionary size : " + str(len(soldic)) )
        #debug
        iterbestelement[:] = []
        ...
        if len(wallofshame)>0:
            for m in range(0, int(runparameters['POPULATION_SIZE'] )): #todo REPARAR
                repulsorchampion[:] = []
                repulsorchampion=repulsortournment(population,trainingset,runparameters['REPULSOR_TOURNMENT_SIZE'],wallofshame,'euclidean')
