...
#This is an implementation that only learns, does not test neither validate, used for performance and
#result verification

def exec_GP_ClassicLearnOnly(runparameters, _):
    ...
    trainingset,testset=tmpdataset[:],tmpdataset[:]
    ...
    for n in range (1,runparameters['MAX_GENERATIONS']+1):  #Repeat iterations
        ...
        #graphsave(bestelement, -50, 50, 1, str(n) + "_" + str(m),RUN_ID)
        saveplotregressionresults(runbestelement, trainingset, str(n), runparameters['RUN_ID'])
        saveresulttable(runbestelement, trainingset, "train__" + str(n), runparameters['RUN_ID'])
        saveresulttable(runbestelement, testset, "test__" + str(n), runparameters['RUN_ID'])
        if runparameters['SAVE_GRAPHS']:
            create_graph(runbestelement, str(runparameters['RUN_ID']) + '/graphs/' + str(n) + '.dot')
    ...
    print("------------------------")
    print(runbestelement)
    #graph(target,-50,50,1,RUN_ID)
    #graph(bestelement,-50,50,1,RUN_ID)
    ...
    if runparameters['SAVE_PLOTS']:
        ...
        saveresulttable(runbestelement, trainingset, "Final_Training_" + str(n) + "_" + str(m), runparameters['RUN_ID'])
        saveresulttable(runbestelement, testset, "Final_Test_" + str(n) + "_" + str(m), runparameters['RUN_ID'])
    ...
    # outfile.write('localtime: ' + str(time.localtime()) + '\n')
    # outfile.write('runtime:' + str(time.time()-runparameters['START_TIME']) + '\n')
    # outfile.write('Fit at iteration :' + str(n) + "=" + str(iterbestfit) + "\n")
    # outfile.write("best iteration :" + iterrpnbest + "\n")
    # outfile.write(str(iterbestelement) + '\n')
    # outfile.write('best fit at iteration :' + str(n) + "=" + str(bestfit) + "\n")
    # outfile.write(str(bestelement) + '\n')
    # outfile.close()


    # if bestfit>cyclebestelementfitness:
    #     bestfit=cyclebestelementfitness
    #     bestelement=cyclebestelement[:]
    #     print(str(m) + ":" + str(n) + ">>>" + str(bestfit))
    #     if runparameters['SAVE_PLOTS']:
    #         saveplotregressionresults(bestelement, trainingset, "++" + str(n) + "_" + str(m), runparameters['RUN_ID'])
    #     SaveResultTable(bestelement, trainingset, "train__" + str(n) + "_" + str(m), runparameters['RUN_ID'])
    #    # dblog_insertgpjobregressioniteration(JOBDBID, RUN_ID, n, bestelement, trainingset)
    #     if runparameters['SAVE_GRAPHS']:
    #         create_graph(bestelement, str(runparameters['RUN_ID']) + '/graphs/' + str(n) + '.dot')
    #
    # if iterbestfit > cyclebestelementfitness:
    #     iterbestfit = cyclebestelementfitness
    #     iterbestelement = cyclebestelement[:]
    #     iterbesttestfitness = getfitness(iterbestelement, testset, {}, False)